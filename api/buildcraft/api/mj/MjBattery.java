 ...
 * {@link #tick(Level, Vector3d)} every tick to allow for losing excess power. */
public class MjBattery implements INBTSerializable<CompoundTag> {
    ...
    public void tick(Level world, Vector3d position) {
        if (microJoules > capacity * 2) {
            losePower(world, position);
        }
    }

    protected void losePower(Level world, Vector3d position) {
