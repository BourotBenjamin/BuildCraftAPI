 ...
 * {@link net.minecraftforge.common.capabilities.RegisterCapabilitiesEvent#register(Class)} doesn't return the registered {@link Capability}
 ...
 * via the forge-recommended {@link Capability} route, or refer to the various fields in buildcraft api classes
...
public class CapabilitiesHelper {
    ...
    /** A type of {@link Capability} that contains the class that it would store. Used by the internal mechanisms of
    ...
    public static abstract class CheckedStorage<T> {
        ...
        public abstract Tag writeNBT(Capability<T> capability, T instance, FaceInfo side);
        public abstract void readNBT(Capability<T> capability, T instance, FaceInfo side, Tag nbt);
    }
    ...
    public static final class ThrowingStorage<T> extends CheckedStorage<T> {
        ...
        @Override
        public Tag writeNBT(Capability<T> capability, T instance, FaceInfo side) {
            throw new UnsupportedOperationException("You must create your own instances!");
        }

        @Override
        public void readNBT(Capability<T> capability, T instance, FaceInfo side, Tag nbt) {
            throw new UnsupportedOperationException("You must create your own instances!");
        }
    }
    ...
    private static <T> Capability<T> registerCapInternal(CheckedStorage<T> storage, Callable<T> factory) {
        ...
        // TODO : Fix CapabilityManager.INSTANCE.register(clazz, storage, factory);
