...
public interface IFlowFluid {
    ...
    @Nullable
    @Deprecated
    default FluidStack tryExtractFluid(int millibuckets, FaceInfo from, FluidStack filter) {
    ...
    @Nullable
    FluidStack tryExtractFluid(int millibuckets, FaceInfo from, FluidStack filter, boolean simulate);

    /** @deprecated use the version below with a simulate paramater. */
    @Deprecated
    default FluidActionResult tryExtractFluidAdv(int millibuckets, FaceInfo from, IFluidFilter filter) {
        return tryExtractFluidAdv(millibuckets, from, filter, false);
    }

    /** Advanced version of {@link #tryExtractFluid(int, FaceInfo, FluidStack, boolean)}. Note that this only works for
     * instances of {@link IFluidHandler} that ALSO extends {@link IFluidHandlerAdv}
     * 
     * @param millibuckets
     * @param from
     * @param filter A filter to try and match fluids.
     * @return The fluidstack extracted and inserted into the pipe. If {@link FluidActionResult#isSuccess()} ()} equals
     *         {@link FluidActionResult#FAILURE} then it means that the {@link IFluidHandler} didn't implement
     *         {@link IFluidHandlerAdv} and you should call the basic version, if you can. */
    FluidActionResult tryExtractFluidAdv(int millibuckets, FaceInfo from, IFluidFilter filter, boolean simulate);
    ...
    int insertFluidsForce(FluidStack fluid, @Nullable FaceInfo from, boolean simulate);
     ...
     * @return */
    @Nullable
    FluidStack extractFluidsForce(int min, int max, @Nullable FaceInfo section, boolean simulate);
}
